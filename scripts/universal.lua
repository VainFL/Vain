--This watermark is used to delete the file if its cached, remove it to make the file persist after vain updates.
--This watermark is used to delete the file if its cached, remove it to make the file persist after vain updates.
--This watermark is used to delete the file if its cached, remove it to make the file persist after vain updates.
--This watermark is used to delete the file if its cached, remove it to make the file persist after vain updates.
--This watermark is used to delete the file if its cached, remove it to make the file persist after vain updates.
...
local function serverHop(pointer, filter)
	...
	if data and data.data then
		...
		notif('vain', 'Failed to grab servers. ('..(data and data.errors[1].message or 'no data')..')', 5, 'warning')
	end
end
...
vain:Clean(lplr.OnTeleport:Connect(function()
	if not tpSwitch then
		tpSwitch = true
		queue_on_teleport("shared.vainserverhoplist = '"..table.concat(visited, '/').."'\nshared.vainserverhopprevious = '"..game.JobId.."'")
	end
end))
...
run(function()
	...
	vain:Clean(vain.Categories.Friends.Update.Event:Connect(function() entitylib.refresh() end))
	vain:Clean(vain.Categories.Targets.Update.Event:Connect(function() entitylib.refresh() end))
...
run(function()
	...
	function whitelist:process(msg, plr)
		...
		if self.localprio > 0 and not self.said[plr.Name] and msg == 'hello nice vibe' and plr ~= lplr then
			...
			notif('vain', plr.Name..' is using vain!', 60)
	...
	function whitelist:hook()
		...
		if textChatService.ChatVersion == Enum.ChatVersion.TextChatService then
			if exp and exp:WaitForChild('appLayout', 5) then
				vain:Clean(exp:FindFirstChild('RCTScrollContentView', true).ChildAdded:Connect(function(obj)
		...
		if exp then
			...
			if bubblechat then
				vain:Clean(bubblechat.DescendantAdded:Connect(function(newbubble)
	...
	function whitelist:update(first)
		local suc = pcall(function()
			...
			whitelist.textdata = readfile('vain/profiles/whitelist.json')--game:HttpGet('https://raw.githubusercontent.com/VainFL/Vain/vain/newwhitelist'..commit..'/t.json', true)
		end)
...
run(function()
	for _, channel in pairs(textChatService:WaitForChild("TextChannels", 9e9):GetChildren()) do
		vain:Clean(channel.MessageReceived:Connect(function(message)
...
run(function()
	...
	local function Added(ent)
		...
		EntityArrow.Image = getcustomasset('vain/assets/new/arrowmodule.png')
	...
	GamingChair = vain.Categories.Render:CreateModule({
		...
		Function = function(callback)
			if callback then
				...
				--movingsound.SoundId = downloadvainAsset('vain/assets/ChairRolling.mp3')
				movingsound.Volume = 0.4
				movingsound.Looped = true
				movingsound.Parent = workspace
				flyingsound = Instance.new('Sound')
				--flyingsound.SoundId = downloadvainAsset('vain/assets/ChairFlying.mp3')
				...
				repeat
					if entitylib.isAlive and entitylib.character.Humanoid.Health > 0 then
						...
						local flying = vain.Modules.Fly and vain.Modules.Fly.Enabled or vain.Modules.LongJump and vain.Modules.LongJump.Enabled or vain.Modules.InfiniteFly and vain.Modules.InfiniteFly.Enabled
	...
	ChatSpammer = vain.Categories.Utility:CreateModule({
		...
		Function = function(callback)
			if callback then
				...
				local ind = 1
				repeat
					local message = (#Lines.ListEnabled > 0 and Lines.ListEnabled[math.random(1, #Lines.ListEnabled)] or 'vain on top')
